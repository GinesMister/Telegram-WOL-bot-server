...
class ConfigService {
  ...
  loadConfig() {
    if (!this.configPath.toLowerCase().endsWith('.json5')) {
      throw new Error(`The config file must be JSON5 format. File received: ${this.configPath}`);
    }
    try {
      const fileContent = fs.readFileSync(
          path.join(__dirname, '/../', this.configPath), 'utf-8');
      this.userConfig = JSON5.parse(fileContent) as UserConfig;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } catch (error: any) {
      if (error.code === 'ENOENT')
        throw new Error(`Config file not found in this route: ${this.configPath}`, { cause: error });
